...
> **Note:** The flame graph SVG is generated by `scripts/generate_flamegraph.sh` and written to `docs/performance/flamegraph.svg`. It is excluded from version control (see `.gitignore`). Generate it locally with:
> ```bash
